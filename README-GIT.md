...
Your first step is to establish a public repository from which i can pull your work into the master repository.
 ...
 2. Fork the vagrant-librarian-puppet repository (http://github.com/sandrokeil/vagrant-librarian-puppet)
 3. Clone your fork locally and enter it (use your own GitHub username in the statement below)

    ```sh
    % git clone git@github.com:<username>/vagrant-librarian-puppet.git
    % cd vagrant-librarian-puppet
    ```

 4. Add a remote to the canonical vagrant-librarian-puppet repository, so you can keep your fork
    up-to-date:

    ```sh
    % git remote add upstream https://github.com/sandrokeil/vagrant-librarian-puppet.git
    % git fetch upstream
    ```

## Keeping Up-to-Date

Periodically, you should update your fork to match the canonical vagrant-librarian-puppet repository. we have
added a remote to the vagrant-librarian-puppet repository, which allows you to do the following:
...
## Working on vagrant-librarian-puppet

When working on vagrant-librarian-puppet, we recommend you do each new feature or bugfix in a new branch. This simplifies the
...
To ssh://git@github.com/sandrokeil/vagrant-librarian-puppet.git
...
`https://github.com/sandrokeil/vagrant-librarian-puppet/commits/<branch>.atom`

where &lt;branch&gt; is a branch in the repository.

To subscribe to git email notifications, simply watch or fork the vagrant-librarian-puppet repository on GitHub.
